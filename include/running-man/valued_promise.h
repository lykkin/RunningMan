...
enum class PromiseStates { Pending, Fulfilled, Rejected };
...
public:
  template <typename U = void> Promise<U> then(std::function<U(T)>);
  inline Promise<T>(callback_t<T>);
  inline ~Promise<T>();
};

template <typename T, typename U> struct thenExec {
  static void exec(std::function<U(T)> f, T v, resolve_cb_t<U> resolve) {
    resolve(f(v));
  };
};

template <typename T> struct thenExec<T, void> {
...
template <typename T> inline Promise<T>::Promise(callback_t<T> callback) {
  data = std::make_shared<PromiseData>();
  data->id = promise_id++;
  try {
    callback(
        [data{this->data}](auto v) {
